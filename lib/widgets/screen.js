...
Screen.prototype._tcC = (x, y) => `${x}x${y}`;
Screen.prototype.addColorElem = function(elem) {
  if (!elem) return false;
  if (!(elem instanceof Element)) return false;
  if (!elem.style.truec) return false;
  for (let x = elem.aleft; x < elem.aleft + elem.width; x++) {
    for (let y = elem.atop; y < elem.atop + elem.height; y++) {
      this._truec.set(this._tcC(x, y), elem.style.truec);
    }
  }
  console.error(this._truec);
}
Screen.prototype.removeColorElem = function(elem) {
  if (!elem) return false;
  if (!(elem instanceof Element)) return false;
  for (let x = elem._getLeft(); x < elem._getLeft() + elem._getWidth(); x++) {
    for (let y = elem._getTop(); y < elem._getTop() + elem._getHeight(); y++) {
      if (map.get(this._tcC(x, y))) map.remove(this._tcC(x, y));
    }
  }
}
...
Screen.prototype.draw = function(start, end) {
  ...
  for (y = start; y <= end; y++) {
    ...
    for (x = 0; x < line.length; x++) {
      ...
      let truec = false;
      if (this._truec.get(this._tcC(x, y))) {
        const { r, g, b } = this._truec.get(this._tcC(x, y));
        truec = `48;2;${r};${g};${b};`
      }
      if (data !== attr) {
        if (attr !== this.dattr) {
